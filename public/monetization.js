(function cardcraftMonetization(){
  ...
  const cfg={
    premiumPrice:1900,currency:'KRW',
  ...
  let session={price:Number(cfg.premiumPrice)||1900,currency:cfg.currency||'KRW',paymentLive:false,adLive:false};
  ...
  modal.innerHTML=`
    <section class="cc-paywall-card" role="dialog" aria-modal="true" aria-labelledby="ccPaywallTitle">
      ...
      <div class="cc-paywall-body">
        ...
        <div class="cc-demo-flag" id="ccDemoFlag">현재 테스트 모드입니다.</div>
  ...
  const titleEl=modal.querySelector('#ccPaywallTitle'),descEl=modal.querySelector('#ccPaywallDesc'),kickerEl=modal.querySelector('#ccKicker'),metaLabel=modal.querySelector('#ccMetaLabel'),metaEl=modal.querySelector('#ccDownloadMeta'),choice=modal.querySelector('#ccPrimaryChoice'),choiceIcon=modal.querySelector('#ccChoiceIcon'),choiceTitle=modal.querySelector('#ccChoiceTitle'),choiceDesc=modal.querySelector('#ccChoiceDesc'),choicePrice=modal.querySelector('#ccChoicePrice'),choiceGrid=modal.querySelector('#ccChoiceGrid'),progress=modal.querySelector('#ccProgress'),progressTitle=modal.querySelector('#ccProgressTitle'),progressText=modal.querySelector('#ccProgressText'),adStage=modal.querySelector('#ccAdStage'),adCount=modal.querySelector('#ccAdCount'),errorBox=modal.querySelector('#ccError'),noteEl=modal.querySelector('#ccNote'),demoFlag=modal.querySelector('#ccDemoFlag');
  ...
  async function loadSession(){try{session={...session,...await fetchJson(cfg.endpoints.session,{method:'GET'})};}catch{}if(opened)setGate(gateMode);}

  function triggerDesignGeneration(){const button=document.getElementById('v10AiRun');if(!button)return;bypassDesign=true;closeGate(true);requestAnimationFrame(()=>button.click());}
  ...
  async function liveRewardedAd(){
    ...
    try{
      ...
      await new Promise((resolve,reject)=>{
        const gt=window.googletag;let settled=false,slot=null,granted=false;
        ...
        gt.cmd.push(()=>{
          ...
          gt.pubads().addEventListener('rewardedSlotGranted',event=>{if(event.slot!==slot)return;granted=true;finish(resolve,true);});
          gt.pubads().addEventListener('rewardedSlotClosed',event=>{if(event.slot!==slot||granted)return;setTimeout(()=>finish(reject,new Error('AD_CLOSED')),0);});
          gt.enableServices();gt.display(slot);
        });
        setTimeout(()=>finish(reject,new Error('AD_TIMEOUT')),20000);
      });
