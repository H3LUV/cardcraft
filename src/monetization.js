...
export async function handleMonetizationRequest(request,env,headers={}){
  ...
  if(pathname==='/api/session'){
    ...
    return json({
      ...
      price:priceFrom(env),
      currency:'KRW',
      unlockHours:Math.max(1,Number(env.CARDCRAFT_UNLOCK_HOURS)||24),
      paymentLive:mode==='live',
      adLive:String(env.CARDCRAFT_REWARDED_AD_LIVE||'false').toLowerCase()==='true'
    },200,headers);
  }
  ...
  if(pathname==='/api/payments/orders'){
    ...
    if(!body)return json({error:'INVALID_JSON'},400,headers);
    const paymentId=randomId('cardcraft');
    return json({
      ok:true,
      paymentId,
      orderName:'Cardcraft 24시간 다운로드 패스',
      amount:priceFrom(env),
      currency:'KRW'
    },200,headers);
  }
  ...
  if(pathname==='/api/payments/complete'){
    ...
    const mode=modeFrom(env);
    if(mode!=='live'){
      return json({ok:true,granted:true,mode:'demo',unlockHours:Math.max(1,Number(env.CARDCRAFT_UNLOCK_HOURS)||24)},200,headers);
    }
    try{
      const payment=await verifyPortOne(paymentId,env);
      const paidAmount=Number(payment?.amount?.total ?? payment?.amount?.paid ?? 0);
      const expected=priceFrom(env);
      ...
      return json({ok:true,granted:true,mode:'live',unlockHours:Math.max(1,Number(env.CARDCRAFT_UNLOCK_HOURS)||24),paymentId},200,headers);
    }catch(error){
